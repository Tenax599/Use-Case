$(document).ready(function () {
  ...
  $("#student-list").on("click",".editStudentBtn", function () {
    ...
    $.ajax({
      type: "GET",
      url: url.teacherAdmin.edit, // 用于获取学生信息的PHP文件
      data: { student_id: studentId },
      dataType: "json",
      success: function (response) {
        console.log(response);
        // 填充模态框表单
        $("#studentId").val(response.job_number);
        $("#Student_name").val(response.name);
        $("#Student_gender").val(response.gender);
        $("#Student_class").val(response.birthdate);
        $("#Student_avatar").val(response.avatarUrl);
        $("#teacher_major").val(response.major);
        $("#teacher_position").val(response.position);
        // 其他表单字段填充...

        // 显示模态框
        const editStudentModal = new bootstrap.Modal('#editStudentModal', {
          keyboard: false
        })
  ...
  $("#updateStudentBtn").on("click", function () {
    ...
    $.ajax({
      type: "POST",
      url: url.teacherAdmin.update, // 用于更新学生信息的PHP文件
      data: {
